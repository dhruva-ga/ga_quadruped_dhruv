import zmq, struct, time

FMT = "<13f"  # ch1..ch13, normalized [-1, 1]

class SbusVelocityController:
    """
    ZMQ/SBUS-driven velocity controller (event-in, no polling from keyboard).

    Channel mapping (1-indexed in your YAML, 0-indexed in payload):
      ch2 (r_fwd_back)  -> w
      ch3 (l_fwd_back)  -> vx
      ch4 (l_left_right)-> vy
    """

    def __init__(
        self,
        endpoint="tcp://localhost:8011",
        topic=b"sbus",
        vmax_lin=1.0,
        vmax_ang=1.0,
        deadzone=0.03,
        invert_left_vertical=True,   # set True if pushing stick forward reads as -1
        invert_right_vertical=True,  # same note for r_fwd_back -> w
    ):
        self.vmax_lin = float(vmax_lin)
        self.vmax_ang = float(vmax_ang)
        self.deadzone = float(deadzone)
        self.invert_left_vertical = bool(invert_left_vertical)
        self.invert_right_vertical = bool(invert_right_vertical)

        # state
        self.vx = 0.0
        self.vy = 0.0
        self.w  = 0.0
        self._last_msg_time = 0.0

        # ZMQ sub socket
        self._ctx = zmq.Context.instance()
        self._sub = self._ctx.socket(zmq.SUB)
        self._sub.connect(endpoint)
        self._sub.setsockopt(zmq.SUBSCRIBE, topic)

        # non-blocking poller (so step() can be called in your control loop)
        self._poller = zmq.Poller()
        self._poller.register(self._sub, zmq.POLLIN)

    # --- public API (mirrors VelocityController) -----------------------------

    def step(self, timeout_ms=0):
        """
        Pump at most one incoming SBUS message (non-blocking by default),
        update (vx, vy, w), and return them.
        """
        events = dict(self._poller.poll(timeout_ms))
        if self._sub in events and events[self._sub] == zmq.POLLIN:
            try:
                topic, header, payload = self._sub.recv_multipart(zmq.NOBLOCK)
            except zmq.Again:
                return self.vx, self.vy, self.w  # nothing new

            if not payload or len(payload) != struct.calcsize(FMT):
                return self.vx, self.vy, self.w  # malformed; keep last

            vals = struct.unpack(FMT, payload)  # tuple of 13 floats

            # --- extract channels (0-based indices for ch1..ch13) -----------
            r_left_right  = vals[0]  # ch1 (unused here)
            r_fwd_back    = vals[1]  # ch2 -> w
            l_fwd_back    = vals[2]  # ch3 -> vx
            l_left_right  = vals[3]  # ch4 -> vy
            # others available in vals[4:] if you need them later

            # optional axis inversion (common that "forward" = negative)
            if self.invert_left_vertical:
                l_fwd_back = -l_fwd_back
            if self.invert_right_vertical:
                r_fwd_back = -r_fwd_back

            # deadzone
            l_fwd_back   = 0.0 if abs(l_fwd_back)   < self.deadzone else l_fwd_back
            l_left_right = 0.0 if abs(l_left_right) < self.deadzone else l_left_right
            r_fwd_back   = 0.0 if abs(r_fwd_back)   < self.deadzone else r_fwd_back

            # scale to commanded velocities
            self.vx = float(l_fwd_back)   * self.vmax_lin
            self.vy = float(l_left_right) * self.vmax_lin
            self.w  = float(r_fwd_back)   * self.vmax_ang

            self._last_msg_time = time.time()

        return self.vx, self.vy, self.w

    def get(self):
        """Return current (vx, vy, w)."""
        return self.vx, self.vy, self.w

    def reset(self):
        """Zero out all commands."""
        self.vx = self.vy = self.w = 0.0

    def __repr__(self):
        return f"SbusVelocityController(vx={self.vx:.3f}, vy={self.vy:.3f}, w={self.w:.3f})"


if __name__ == "__main__":
    # simple test/demo
    ctrl = SbusVelocityController(
        vmax_lin=1.0,   # m/s
        vmax_ang=1.0,   # rad/s
        deadzone=0.05,
        invert_left_vertical=False,
        invert_right_vertical=False,
    )

    # in your control loop:
    while True:
        vx, vy, w = ctrl.step(timeout_ms=0)  # non-blocking
        print(vx, vy, w)